...
Линия: один `strokeYaw`, **Smart Yaw только на последней клетке**. Если подземный конвейер уже открыт и жест пересекает занятые клетки, в разрыв ставится пара входа/выхода (зазор ≤ 5). Призраки (одиночный, линия, копия) считают маску через `Conveyor.PreviewExits`. Стоящая лента, в которую смотрит гост, на превью тоже меняет форму (`ShowIncomingVisual`), симуляция груза не трогается.
